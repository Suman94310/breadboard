export default class Xor{
    ...
    draw = ()=>{
        ...
        if(!this.selected){
            ...
            xor = document.getElementById("xor1")
        }
        this.simulation.context.drawImage(xor, this.position.x, this.position.y, 94, 75);
    }

    update = ()=>{
        if(this.inputIds[0]!=undefined && this.inputIds[1]!=undefined)
            this.outputs[0] = (this.simulation.components[this.inputIds[0]].outputs[this.inputNodes[0]] && !this.simulation.components[this.inputIds[1]].outputs[this.inputNodes[1]]) || (!this.simulation.components[this.inputIds[0]].outputs[this.inputNodes[0]] && this.simulation.components[this.inputIds[1]].outputs[this.inputNodes[1]])
        this.draw()
    }
}